...
import { Button, Card, Header, Icon, Modal, Input } from 'semantic-ui-react';
import './listview.css';
...
function Listview() {
    ...
    useEffect(() => {
        setLoader(true);
        const url = 'https://api.github.com/search/repositories?q=stars:>1+language:javascript&sort=stars&order=desc';
        fetch(url)
            .then((response) => {
    ...
    useEffect(()=>{
        let findNoData = favorite.find((li)=>li.name.includes(searchdata));
        setNoDataFound(findNoData || searchdata === "" ? false : true)
    },[searchdata])
    ...
    return (
        <div className='style-inputbox'>
            ...
            <>
            <Input icon='search' placeholder='Search by Github name' onChange={handleSearch} />
            <div className='full-ctn'>
                {noDataFound && <h3 className='nodata-found'>No Result Found</h3>}
                {favorite.map((data, index) => {
                    return (
                        data.name.includes(searchdata) || searchdata === "" ?
                            <Card key={index}>
